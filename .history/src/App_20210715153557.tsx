...
const toggleRepoStar = (repoId: string, alreadyStarred: boolean) => {
  return axiosGihubGraphQL.post('', {
    query: alreadyStarred ? REMOVE_STAR : ADD_STAR,
    variables: { repoId }
  });
}
...
function App() {
  ...
  const handleStar = useCallback((repoId: string, isAlreadyStarred: boolean) => {
    const mutationType = isAlreadyStarred ? "removeStar": "addStar";
    toggleRepoStar(repoId, isAlreadyStarred).then(res => {
      const { data: _data, errors: _errors  } = res.data;
      if (_data) {
        const { viewerHasStarred } = _data.data[mutationType].starrable;
        setOrganisation((_org: any) => {
          return {
            ..._org,
            repository: {
              ..._org?.repository,
