  ...
  const fetchData = useCallback((path: string, cursor?: string) => {
    getIssuesOfRepo(path, cursor)
      .then(res => {
        ...
        if (_data) {
          if (!cursor || !organisation) {
            setOrganisation(_data.organization);
          }
        } else {
          const mergedIssues = [...organisation!.repository.issues.edges, ..._data.organisation.repository.issues.edges];
          setOrganisation({
            ..._data.organisation,
            repository: {
              ..._data.organisation.repository,
              issues: {
                ..._data.organisation.repository.issues,
                edges: mergedIssues
              }
            }
          });
